...
var Message = {
  init: function(app){
  console.log("init message service begin");
    app.post('/message/get', this.getMessage);
    app.post('/message/add', this.addMessage);
     console.log("init message service end");
  },

  //获取公告消息
  getMessage: function(req, res){
    var key = req.param('key');
    console.log("getMessage,key="+key);
    if(key !== util.getKey()){
      return res.send({
        status: 0,
        data: '使用了没有鉴权的key'
  ...
  addMessage: function(req, res){
    ...
    var message = req.param('message');
     console.log("addMessage,message="+message+" token="+token);
    if(!token || !message){
      return res.send({
        status: 0,
