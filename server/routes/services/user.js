...
var User = {
  ...
  init: function(app){
   console.log("init user service begin");
    ...
    console.log("init user service end");
  },
  ...
  getUser: function(req, res){
    ...
    var partment = req.param('partment');
     console.log("getUser,key="+key+" partment="+partment);
    if(key !== util.getKey()){
      return res.send({
        status: 0,
        data: '使用了没有鉴权的key'
  ...
  addUser: function(req, res){
    ...
    var creater = req.param('creater') || '';
     ...
     console.log("addUser,username="+username+" password="+password+" tel="+tel+" email="+email+" partment="+partment+" tag="+tag+" creater="+creater);
  ...
  login: function(req, res){
    ...
    var deviceId = req.param('deviceId');
      console.log("login,email="+email+" password="+password+" deviceId="+deviceId);
  ...
  updatePassword: function(req, res){
    ...
    var password = util.md5(req.param('password'));
 console.log("updatePassword,token="+token+" oldPassword="+oldPassword+" password="+password);
    var content = JSON.parse(fs.readFileSync(USER_PATH));
    for(var i in content){
  ...
  deleteUser: function(req, res) {
    var token = req.param('token');
    var email = req.param('email');
console.log("deleteUser,token="+token+" email="+email);
    var content = JSON.parse(fs.readFileSync(USER_PATH));
    for (var i in content) {
      if (token === content[i].token) {
        //遍历查找需要删除的用户
